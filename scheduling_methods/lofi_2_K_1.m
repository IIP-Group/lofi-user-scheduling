...
% (iv) Pick the schedule with the largest minimal per-UE SINR, i.e.,
% max(min(SINR_u)), u=1,...,U
% The dimensions of C_matrix are # of UEs x # of timeslots x # of candidate
% configurations, which in this case is 2 for the two schedules. We
...
% =========================================================================
function [C_chosen] = lofi_2_K_1(par,var)
...
while improvement == 1
    ...
    [~,idx_chosen] = max([min([min_candidate_1_1,min_candidate_1_2]),min_candidate_2]);

    if idx_chosen == 2
        % swap improved --> continue
        % the new status quo is the set 2
        SINR_u(:, 1) = SINR_u(:, 2);
        chosen_ues_1 = chosen_ues_2;
        complement_1 = complement_2;
        C_matrix(:,:,1) = C_matrix(:,:,2);
        C_matrix(:,:,2) = 0;
    else
        % swap disimproved --> stop
        improvement = 0;
    end
end

% loop ends after there is no more improvement
C_chosen = C_matrix(:,:,idx_chosen);  % idx_chosen must be 1
